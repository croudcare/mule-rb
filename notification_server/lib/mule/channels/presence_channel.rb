...
module Mule
  class PresenceChannel < Channel
    ...
    def dispatch(message, channel)
      if channel =~ /^Mule:/
        # Messages received from the Redis channel Mule:*  carry info on
    ...
    def initialize(attrs)
      super
      # Also subscribe the Mule daemon to a Redis channel used for events concerning subscriptions.
      Mule::Redis.subscribe 'Mule:connection_notification'
    end
    ...
    def subscribe(msg, callback, &blk)
      ...
      # Send event about the new subscription to the Redis Mule:connection_notification Channel.
    ...
    def roster_add(key, value)
      ...
      Mule::Redis.hset(channel_id, key, value)
    end

    def roster_remove(key)
      # Remove subscription info from Redis.
      Mule::Redis.hdel(channel_id, key)
    end

    def publish_connection_notification(payload, retry_count=0)
      # Send a subscription notification to the global Mule:connection_notification
      # channel.
      Mule::Redis.publish('Mule:connection_notification', payload.to_json).
