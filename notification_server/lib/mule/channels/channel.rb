...
module Mule
  class Channel
    ...
    def initialize(attrs)
      super
      Mule::Redis.subscribe channel_id
    end
    ...
    def subscribe *a, &blk
      Mule::Redis.hincrby('channel_subscriber_count', channel_id, 1).
        callback do |value|
          Mule::Webhook.post name: 'channel_occupied', channel: channel_id if value == 1
        end
    ...
    def unsubscribe *a, &blk
      Mule::Redis.hincrby('channel_subscriber_count', channel_id, -1).
        callback do |value|
          Mule::Webhook.post name: 'channel_vacated', channel: channel_id if value == 0
        end
    ...
    def send_client_message(message)
      Mule::Redis.publish(message['channel'], message.to_json) if authenticated?
    end

    # Send an event received from Redis to the EventMachine channel
    # which will send it to subscribed clients.
    def dispatch(message, channel)
      push(message.to_json) unless channel =~ /^Mule:/
    end
